...
int Arena::numberOfRatsAt(int r, int c) const
{
	...
	for (int k = 0; k < m_nRats; k++)
	{
		if (m_rats[k]->row() == r && m_rats[k]->col() == c)
			number++;
	}
	return number;
}
	...
	for (r = 1; r <= rows(); r++)
		for (c = 1; c <= cols(); c++)
		{
			...
			if (numberOfRatsAt(r, c) > 2 && numberOfRatsAt(r, c) < 9)
				displayGrid[r - 1][c - 1] = numberOfRatsAt(r, c);
...
void Arena::moveRats()
{
	...
	{
		m_rats[k]->move();
		if (m_player->row() == m_rats[k]->row() && m_player->col() == m_rats[k]->col())
			m_player->setDead();
		if (m_rats[k]->isDead() == true)
		{
			delete m_rats[k];
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
	...
	int stay = 0; //COMEBACK
	if (r - 1 >= 0)
		stay += a.numberOfRatsAt(r - 1, c);
	if (r - 1 >= 0 && c + 1 <= a.cols())
		stay += a.numberOfRatsAt(r - 1, c + 1);
	if (c + 1 <= a.cols())
		stay += a.numberOfRatsAt(r, c + 1);
	...
	int south = 0;
	if (r_south - 1 >= 0)
		south += a.numberOfRatsAt(r_south - 1, c);
	if (r_south - 1 >= 0 && c + 1 <= a.cols())
		south += a.numberOfRatsAt(r_south - 1, c + 1);
	if (c + 1 <= a.cols())
		south += a.numberOfRatsAt(r_south, c + 1);
	if (r_south + 1 <= a.rows() && c + 1 <= a.cols())
		south += a.numberOfRatsAt(r_south + 1, c + 1);
	if (r_south + 1 <= a.rows())
		south += a.numberOfRatsAt(r_south + 1, c);
	if (r_south + 1 <= a.rows() && c - 1 >= 0)
		south += a.numberOfRatsAt(r_south + 1, c - 1);
	if (c - 1 >= 0)
		south += a.numberOfRatsAt(r_south, c - 1);
	if (r_south - 1 >= 0 && c - 1 >= 0)
		south += a.numberOfRatsAt(r_south - 1, c - 1);
	int r_north = r - 1;
	int north = 0;
	if (r_north - 1 >= 0)
		north += a.numberOfRatsAt(r_north - 1, c);
	if (r_north - 1 >= 0 && c + 1 <= a.cols())
		north += a.numberOfRatsAt(r_north - 1, c + 1);
	if (c + 1 <= a.cols())
		north += a.numberOfRatsAt(r_north, c + 1);
	if (r_north + 1 <= a.rows() && c + 1 <= a.cols())
		north += a.numberOfRatsAt(r_north + 1, c + 1);
	if (r_north + 1 <= a.rows())
		north += a.numberOfRatsAt(r_north + 1, c);
	if (r_north + 1 <= a.rows() && c - 1 >= 0)
		north += a.numberOfRatsAt(r_north + 1, c - 1);
	if (c - 1 >= 0)
		north += a.numberOfRatsAt(r_north, c - 1);
	if (r_north - 1 >= 0 && c - 1 >= 0)
		north += a.numberOfRatsAt(r_north - 1, c - 1);
	int c_west = c - 1;
	int west = 0;
	if (r - 1 >= 0)
		west += a.numberOfRatsAt(r - 1, c_west);
	if (r - 1 >= 0 && c_west + 1 <= a.cols())
		west += a.numberOfRatsAt(r - 1, c_west + 1);
	if (c_west + 1 <= a.cols())
		west += a.numberOfRatsAt(r, c_west + 1);
	if (r + 1 <= a.rows() && c_west + 1 <= a.cols())
		west += a.numberOfRatsAt(r + 1, c_west + 1);
	if (r + 1 <= a.rows())
		west += a.numberOfRatsAt(r + 1, c_west);
	if (r + 1 <= a.rows() && c_west - 1 >= 0)
		west += a.numberOfRatsAt(r + 1, c_west - 1);
	if (c_west - 1 >= 0)
		west += a.numberOfRatsAt(r, c_west - 1);
	if (r - 1 >= 0 && c_west - 1 >= 0)
	...
	int east = 0;
	if (r - 1 >= 0)
		east += a.numberOfRatsAt(r - 1, c_east);
	if (r - 1 >= 0 && c_east + 1 <= a.cols())
		east += a.numberOfRatsAt(r - 1, c_east + 1);
	if (c_east + 1 <= a.cols())
		east += a.numberOfRatsAt(r, c_east + 1);
	if (r + 1 <= a.rows() && c_east + 1 <= a.cols())
		east += a.numberOfRatsAt(r + 1, c_east + 1);
	if (r + 1 <= a.rows())
		east += a.numberOfRatsAt(r + 1, c_east);
	if (r + 1 <= a.rows() && c_east - 1 >= 0)
		east += a.numberOfRatsAt(r + 1, c_east - 1);
	if (c_east - 1 >= 0)
		east += a.numberOfRatsAt(r, c_east - 1);
	if (r - 1 >= 0 && c_east - 1 >= 0)
		east += a.numberOfRatsAt(r - 1, c_east - 1);
	int compare[5] = { north, east, south, west, stay };
	int lowest = 4;
	for (int k = 0; k < 5; k++)
	{
		if (compare[k] < compare[lowest])
			lowest = k;
	}
	if (lowest == 4)
		return false;
